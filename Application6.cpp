...
#define INFILE  "ppot.asc"   //"POT4.ASC"
...
*/
	...
	*/
...
#if 1   /* set up null Filter or valid Filter  */
		for( int i=0; i<AAKERNEL_SIZE; i++ ){
		...
		for( int i=0; i<AAKERNEL_SIZE; i++ ){
			nameListShader[6]  = GZ_AASHIFTX;
			valueListShader[6] = (GzPointer)&zero;
			nameListShader[7]  = GZ_AASHIFTY;
			valueListShader[7] = (GzPointer)&zero;
			status |= GzPutAttribute(m_pRender[i], 8, nameListShader, valueListShader);
		}
#endif
...
{
	//GzDisplay*  sampleDisplay;//[AAKERNEL_SIZE];		// the display
	//GzRender*   sampleRender;//[AAKERNEL_SIZE];		// the renderer 
	//char*		sampleFB;//[AAKERNEL_SIZE];		// Frame Buffer
	//int		    xRes, yRes;
	...
	*/ 
	...
	GzIntensity tmpR = 0, tmpG = 0, tmpB = 0;
	...
	for( int i=0; i<m_pDisplay[0]->xres; i++ ){
		for( int j=0; j<m_pDisplay[0]->yres; j++){
			...
			for( int k=0; k<AAKERNEL_SIZE; k++ ){
				GzGetDisplay( m_pDisplay[k], i, j, &r, &g, &b, &a, &z );
				tmpR += r * AAFilter[k][2];
				tmpG += g * AAFilter[k][2];
				tmpB += b * AAFilter[k][2];
			}
		
			GzPutDisplay(m_pDisplay[0], i, j, tmpR, tmpG, tmpB, a, z);
		}
	}
